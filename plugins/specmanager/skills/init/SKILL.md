---
name: init
description: Scaffold .claude/specs/ in the current project and write the managed SpecManager block into CLAUDE.md. Use when initializing SpecManager for the first time in a repo.
---
...
Call the **`specmanager_init`** MCP tool. It is idempotent — safe to re-run.
After it returns, report what changed: which directories were created, and
whether `CLAUDE.md` was created or updated.

## Don't

- Don't edit files outside the `specmanager:start`/`specmanager:end` markers
  in `CLAUDE.md`. The user owns that content.
