...
import { initProject, createFeature, createTask, updateTask, listPhases, getNextPhase, listTasks, } from "./core/index.js";
...
async function main() {
    ...
    let rejected = false;
    try {
        await createTask({ featureId: feature.id, title: "too big", phase: "A", complexity: 5 }, root);
    }
    catch (err) {
        rejected = true;
        const e = err;
    ...
    const legacyTasksPath = path.join(root, ".claude/specs/features", legacy.slug, "plan", "tasks.json");
    await fs.mkdir(path.dirname(legacyTasksPath), { recursive: true });
    const now = new Date().toISOString();
    await fs.writeFile(legacyTasksPath, JSON.stringify({
        tasks: [
            {
                id: "task-1",
                featureId: legacy.id,
...
//# sourceMappingURL=selftest-phases.js.map