...
async function main() {
    ...
    for (const file of files) {
        ...
        if (second !== first)
            driftFiles.push(path.relative(projectRoot, file));
        if (first.trimEnd() !== body.trimEnd())
            normalized++;
    }
    assert(driftFiles.length === 0, `parse→serialize is idempotent (byte-clean re-save) over all ${files.length} docs` +
        (driftFiles.length ? ` — drift in: ${driftFiles.join(", ")}` : ""));
    console.log(`info — ${normalized}/${files.length} docs would be normalized on their first WYSIWYG save (expected; remark tidies on first pass, then converges).`);
