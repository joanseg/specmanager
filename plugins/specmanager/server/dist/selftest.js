...
async function main() {
    ...
    assert(!/\n---\n/.test(sanitized), "sanitizeDesignBriefBody defangs `---` at column 0");
    assert(sanitized.includes("<!-- --- -->"), "sanitizeDesignBriefBody wraps `---` in an HTML comment");
    ...
    assert(designDoc.frontmatter.dependsOn[0] === prd.frontmatter.id, "design doc depends on PRD");
    ...
    assert(mockupRoundTrip.body.includes('<section class="sm-screen">'), "design doc body round-trips stacked screen sections");
    assert(mockupRoundTrip.body.includes("<style>"), "design doc body round-trips inline styles");
    assert(mockupRoundTrip.body.includes("<!-- --- -->"), "design doc body preserves the sanitized escape on read");
    ...
    const designDir = path.join(root, ".claude/specs/features/checkout-corridor/design");
    await fs.mkdir(designDir, { recursive: true });
    await fs.writeFile(path.join(designDir, "rogue.html"), "<!doctype html><html><body>no frontmatter here</body></html>", "utf8");
    ...
    assert(claudeMd4.includes("_1 feature shipped — full history on the board._"), "CLAUDE.md collapses shipped features into the shipped line");
    assert(!claudeMd4.includes("| Shipped widget |"), "shipped feature has no table row");
    assert(claudeMd4.includes("| Checkout corridor |") && claudeMd4.includes("| Session timeline |"), "in-flight features keep their table rows");
    ...
    assert(claudeMd5.includes("| Shipped widget |") && !claudeMd5.includes("shipped — full history"), "reopening the final walkthrough restores the table row (self-healing)");
