...
import { listDocuments, readDocumentById, writeDocument, } from "./core/index.js";
const SESSIONS = new Map();
const IDLE_MS = 15 * 60 * 1000;
function cleanupIdleSessions() {
    const now = Date.now();
    for (const [id, s] of SESSIONS) {
        if (!s.active && now - s.lastUsed > IDLE_MS)
            SESSIONS.delete(id);
    }
}
...
export function chatStatus() {
    ...
    return {
        available: false,
        reason: "set ANTHROPIC_API_KEY (or run inside an authenticated Claude Code session) — see docs/phase-6-test-walkthrough.md",
    };
}
const STAGE_PERSONA = {
...
function buildSystemPrompt(frontmatter, body, mode) {
    const persona = STAGE_PERSONA[frontmatter.stage];
    const modeLine = mode === "interview"
...
function makeMcpServer(projectRoot) {
    return createSdkMcpServer({
        name: "specmanager-chat",
        version: "0.6.0",
        instructions: "In-process bridge to SpecManager core. Use these tools to read and persist document edits with optimistic concurrency.",
        tools: [
            tool("read_document", "Return a SpecManager document's frontmatter and body by id.", { id: z.string() }, async ({ id }) => {
                try {
                    ...
                    return {
                        content: [
                            {
                                type: "text",
                                text: JSON.stringify({ ...d.frontmatter, body: d.body, filePath: d.filePath }, null, 2),
                            },
                        ],
                    };
                }
            ...
            tool("list_documents", "List documents in the project, optionally filtered by featureId / stage / status / stale.", {
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: JSON.stringify(docs.map((d) => ({ ...d.frontmatter, filePath: d.filePath })), null, 2),
                        },
                    ],
                };
            }),
            tool("write_document", "Persist a new body to a SpecManager document. Pass baseVersion to detect mid-stream conflicts (the user editing in another window).", {
                id: z.string(),
                body: z.string(),
                baseVersion: z.number().int().nonnegative(),
            }, async ({ id, body, baseVersion }) => {
                try {
                    const d = await writeDocument({ id, body, baseVersion, generatedBy: "agent" }, projectRoot);
                    return {
                        content: [
                            {
                                type: "text",
...
export async function runChat(input) {
    if (!chatAvailable()) {
        input.onEvent({
            type: "error",
            reason: "chat is unavailable: no ANTHROPIC_API_KEY / CLAUDE_CODE_OAUTH_TOKEN on the MCP server's environment",
        });
        return;
    }
    ...
    try {
        ...
        input.onEvent({ type: "error", reason: err.message });
        return;
    }
    const mode = input.mode ?? (doc.body.trim().length === 0 ? "interview" : "co-write");
    const systemPrompt = buildSystemPrompt(doc.frontmatter, doc.body, mode);
    const abort = new AbortController();
    const session = { abort: () => abort.abort(), active: true, lastUsed: Date.now() };
...
export function cancelChat(docId) {
