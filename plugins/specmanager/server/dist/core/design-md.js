...
async function walkForComponents(start, rel, out, depth = 0) {
    if (depth > MAX_DEPTH)
        return;
    let entries = [];
    try {
    ...
    for (const e of entries) {
        ...
        if (e.isDir) {
            ...
            out.count++;
            if (out.samples.length < MAX_SAMPLE)
                out.samples.push(childRel);
        }
    }
}
async function harvestCssVars(start, out, depth = 0) {
    if (depth > MAX_DEPTH)
        return;
...
function dedupeNested(dirs) {
    const sorted = [...dirs].sort((a, b) => a.length - b.length);
    const out = [];
    for (const d of sorted) {
        if (!out.some((p) => d === p || d.startsWith(p + path.sep)))
            out.push(d);
    }
    return out;
}
// ─── Render ───────────────────────────────────────────────────────────────
function yamlBlock(digest) {
    const lines = [];
    lines.push("---");
    ...
    const colorVars = Object.entries(digest.cssVars).filter(([, v]) => /^(#[0-9a-fA-F]{3,8}|rgb|hsl|oklch|var\()/.test(v.trim()));
    lines.push("colors:");
    if (colorVars.length > 0) {
        for (const [k, v] of colorVars.slice(0, 12))
            lines.push(`  ${safeKey(k)}: ${quote(v)}`);
    }
    else {
...
function renderBody(digest) {
    const lines = [];
    lines.push("# Design system");
    lines.push("");
    lines.push(`_This file follows the [Stitch DESIGN.md spec](./references/stitch-design-md.md). The block between the managed markers is auto-generated from the repo's UI sources by \`/specmanager-init\` and refreshed after every feature ships. **Anything outside the markers is yours** — write freely._`);
    lines.push("");
    // Overview
    lines.push("## Overview");
    lines.push("");
    if (digest.uiDirs.length === 0) {
        lines.push("_No UI source directory detected yet. Once the project grows a `src/`, `app/`, `ui/`, `web/`, `frontend/`, or `client/` directory with component files, the next re-sync will populate this section._");
    }
    else {
        lines.push(`Inferred from \`${digest.uiDirs.join("`, `")}\` (${digest.componentCount} component file(s)). The project's voice and feel should be described here — replace this paragraph with brand personality, target audience, and the emotional response the UI should evoke.`);
    }
    lines.push("");
    // Colors
    lines.push("## Colors");
    ...
    if (colorVars.length > 0) {
        ...
        lines.push("No color CSS variables detected. Add `:root { --primary: #…; }` declarations in your UI's main stylesheet and re-run `/specmanager-init` (or POST `/api/design/sync`) to populate this section.");
    }
    lines.push("");
    // Typography
    lines.push("## Typography");
    lines.push("");
    lines.push("Typography scale — replace the placeholder body-md entry in the frontmatter with the real scale (headlines, body sizes, labels). Most design systems have 9–15 levels.");
    lines.push("");
    // Layout
    lines.push("## Layout");
    lines.push("");
    lines.push("Layout strategy — grid model, breakpoint(s), max content width, spacing scale. The frontmatter `spacing` keys are the canonical values.");
    lines.push("");
    // Elevation & Depth
    lines.push("## Elevation & Depth");
    lines.push("");
    lines.push("How visual hierarchy is conveyed — shadows, tonal layers, borders, or color contrast. Describe the approach this project takes.");
    lines.push("");
    // Shapes
    lines.push("## Shapes");
    lines.push("");
    lines.push("Corner radii and edge treatments. See `rounded.*` in the frontmatter for the scale.");
    lines.push("");
    // Components
    lines.push("## Components");
    lines.push("");
    if (digest.componentSamples.length > 0) {
        ...
        lines.push("");
        for (const p of digest.componentSamples)
            lines.push(`- \`${p}\``);
        lines.push("");
        lines.push("Define style guidance for the component atoms used in this project (Buttons, Inputs, Cards, Lists, etc.) in the frontmatter `components` map.");
    }
    else {
        lines.push("No component files detected yet. Once UI components exist in the repo, the next re-sync will surface them here.");
    }
    ...
    lines.push("");
    lines.push("- Do treat the frontmatter as the source of truth — components reference it via `{colors.primary}`-style token references.");
    lines.push("- Do keep this file in `./docs/DESIGN.md` so `/specmanager-init` and the auto-refresh can find it.");
    lines.push("- Don't edit content **between** the managed markers by hand — SpecManager rewrites it after every feature ships.");
    lines.push("- Don't move or rename the markers themselves; the merge logic searches for them literally.");
    return lines.join("\n");
}
export function renderDesignMd(digest) {
    const block = `${START}\n${yamlBlock(digest)}\n\n${renderBody(digest)}\n${END}`;
    return block;
}
 ...
 */
function overlayTokens(managedBlock, synthesized) {
...
export async function mergeSynthesizedTokens(rootOrTokens, tokensArg) {
    let root;
    let synthesized;
...
export async function syncDesignMd(rootOrOpts, optsArg) {
    let resolvedRoot;
    let opts;
