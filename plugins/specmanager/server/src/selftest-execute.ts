...
async function main(): Promise<void> {
  ...
  try {
    ...
    missing = true;
    assert((err as Error & { code?: string }).code === "missingArtifact", "missingArtifact code");
  }
  ...
  assert(
    featM2.phases.length === 2 &&
      featM2.phases.every((p) => p.walkthroughId !== null),
    "both phases have walkthrough ids in manifest"
  );
  ...
  assert(
    gateFinal0.reason?.includes("A") && gateFinal0.reason?.includes("B"),
    "final gate reason lists missing phases"
  );
  ...
  assert(
    !gateFinal1.reason?.includes("A,") && gateFinal1.reason?.includes("B"),
    "final gate reason now only lists B"
  );
  ...
  await updateTask(
    { id: legacyTask.id, featureId: legacy.id, status: "done", artifacts: { files: ["src/l.ts"] } },
    root
  );
  // Hand-write a legacy non-phase walkthrough file directly.
  const legacyWtDir = path.join(
    root,
  ...
  assert(
    migrated[0]!.endsWith("phase-default.md"),
    `migration renames to phase-default.md (got ${migrated[0]})`
  );
  ...
  assert(
    legacyDocs[0]!.frontmatter.phase === "default",
    "migrated legacy doc has phase=default in frontmatter"
  );
