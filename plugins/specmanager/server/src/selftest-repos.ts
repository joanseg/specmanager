...
async function makeRepo(
  parent: string,
  name: string,
  files: { claudeMd?: string; designMd?: string }
): Promise<string> {
...
async function main(): Promise<void> {
  ...
  assert(
    uiClaudeBody.includes("Read-only mirror"),
    "seeded mirror carries the read-only banner"
  );
  assert(
    uiClaudeBody.includes("Source CLAUDE.md body."),
    "seeded mirror includes the source CLAUDE.md body"
  );

  // 3. hasUi correct per repo (sidecar + scan).
  const uiSidecar = JSON.parse(
    await fs.readFile(path.join(uiTree, ".specmanager-repo.json"), "utf8")
  );
  const noUiSidecar = JSON.parse(
    await fs.readFile(path.join(noUiTree, ".specmanager-repo.json"), "utf8")
  );
  ...
  assert(
    noUiSidecar.sourceHadDesignMd === false,
    "repo-no-ui sidecar records the source had no DESIGN.md"
  );

  const scanned = await scanDeclaredRepos(root);
  assert(scanned.length === 2, "scanDeclaredRepos returns both repos");
  assert(
    scanned.find((r) => r.name === "repo-with-ui")?.hasUi === true,
    "scan reports repo-with-ui hasUi true"
  );
  assert(
    scanned.find((r) => r.name === "repo-no-ui")?.hasUi === false,
    "scan reports repo-no-ui hasUi false"
  );
  ...
  assert(
    claudeMd.includes(
      "- **repo-with-ui** — [CLAUDE.md](./repos/repo-with-ui/CLAUDE.md) · [DESIGN.md](./repos/repo-with-ui/DESIGN.md)"
    ),
    "CLAUDE.md renders the repo-with-ui row with working relative links"
  );
  ...
  assert(
    claudeMd.includes(
      "- **repo-no-ui** — [CLAUDE.md](./repos/repo-no-ui/CLAUDE.md) · [DESIGN.md](./repos/repo-no-ui/DESIGN.md) _(placeholder)_"
    ),
    "CLAUDE.md marks the CLAUDE.md-only repo row as _(placeholder)_"
  );
  ...
  assert(
    uiClaudeAfter === annotated,
    "re-run leaves the annotated nested body byte-identical (un-clobbered)"
  );
  ...
  assert(
    claudeMdAfter.includes(
      "- **repo-third** — [CLAUDE.md](./repos/repo-third/CLAUDE.md) · [DESIGN.md](./repos/repo-third/DESIGN.md)"
    ),
