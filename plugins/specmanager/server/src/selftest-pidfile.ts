...
function hash8(root: string): string {
...
async function main(): Promise<void> {
  ...
  assert(
    pidFilePath(rootA) === path.join(tmp, `board-${hash8(rootA)}.pid`),
    "pidFilePath(root) resolves under ${CLAUDE_PLUGIN_DATA} with a board-<hash8>.pid filename"
  );
  delete process.env.CLAUDE_PLUGIN_DATA;
  assert(
    pidFilePath(rootA) === path.join(os.tmpdir(), `board-${hash8(rootA)}.pid`),
    "pidFilePath(root) falls back to os.tmpdir() when unset"
  );
  ...
  assert(
    pidFilePath(rootA) !== pidFilePath(rootB),
    "pidFilePath() yields distinct paths for distinct project roots"
  );
  ...
  assert(
    Number.parseInt(written.trim(), 10) === process.pid,
    "writePidFile(root) writes the live process PID"
  );
  ...
  await sleep(50);
  assert(
    !isProcessAlive(sleeper.pid),
    "reapStalePid(port, root) SIGTERM'd the live predecessor"
  );
  ...
  assert(
    isProcessAlive(sleeperA.pid),
    "reapStalePid(port, rootB) does not kill rootA's live process"
  );
  const stillA = await fs.readFile(pidFilePath(rootA), "utf8");
  assert(
    Number.parseInt(stillA.trim(), 10) === sleeperA.pid,
    "reapStalePid(port, rootB) leaves rootA's pidfile untouched"
  );
  sleeperA.kill();
  await removePidFile(rootA);
