...
async function main(): Promise<void> {
  ...
  assert(
    pidFilePath() === path.join(tmp, "board.pid"),
    "pidFilePath() resolves under ${CLAUDE_PLUGIN_DATA} when set"
  );
  delete process.env.CLAUDE_PLUGIN_DATA;
  assert(
    pidFilePath() === path.join(os.tmpdir(), "board.pid"),
    "pidFilePath() falls back to os.tmpdir() when unset"
  );
  ...
  assert(
    ...
    "writePidFile() writes the live process PID"
  );
  await removePidFile();
  let stillThere = true;
  try {
  ...
  assert(
    ...
    "reapStalePid SIGTERM'd the live predecessor"
  );
