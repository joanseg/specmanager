 ...
 */
export function isFeatureShipped(
  documents: Array<{ stage: string; status: string; phase?: string }>,
  phases: Array<{ name: string }>
): boolean {
  const approvedWalkthroughs = documents.filter(
    (d) => d.stage === "walkthrough" && d.status === "approved"
  );
  if (approvedWalkthroughs.some((d) => d.phase === "final")) return true;
  if (phases.length === 1) {
