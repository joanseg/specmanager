...
/** Claude Code model aliases — never pinned dated ids (R2/AC2). */
export type Alias = string;

/** Default complexity → tier (R2/AC1). >3 / null / unknown ⇒ strong. */
export const DEFAULT_COMPLEXITY_TO_TIER: Record<number, Tier> = {
  1: "cheap",
  2: "standard",
  3: "strong",
};
...
export function tierForComplexity(complexity: number | null | undefined): Tier {
  if (complexity == null) return "strong";
...
export function aliasForTier(
  tier: Tier,
  sessionTable: Partial<Record<Tier, Alias>> = {}
): Alias {
...
export function aliasForComplexity(
