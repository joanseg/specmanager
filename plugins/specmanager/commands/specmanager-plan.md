---
description: Draft the execution Plan AND emit task records for a SpecManager feature via the planner subagent.
argument-hint: "<featureId or slug>"
---

Generate a Plan + tasks for the feature: **$ARGUMENTS**.
...
4. **Invoke the subagent.** `Task({ subagent_type: "planner", prompt: ... })`. Include:
   ...
   The planner writes `plan.md` AND emits a `create_task` call per Build-order item.
5. **Sync CLAUDE.md.** Call `sync_claude_md`.
6. **Report.** Plan doc id + file path + number of tasks created. Suggest opening the board to see the Build column populated.
