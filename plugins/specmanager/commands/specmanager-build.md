...
7b. **Spec-compliance review (R3) — after the Stop-gate exits 0, before advancing.** The deterministic Stop-gate hook (bash, zero model calls) already gated *stopping*; now run the semantic reviewer before the card advances. Only do this when every task in the phase is `done` (a mid-phase stop skips review). Steps:
   ...
   - **Invoke the reviewer** — its frontmatter fixes its tier regardless of the cards' build tier: `Task({ subagent_type: "reviewer", prompt: <the assembled slice + the phase's commit shas/changed files> })`.
...
8. **Re-resolve phase completion, then run the post-phase pipeline (R2 — driven by persisted state, not the builder's exit path).** After the step-7 dispatch loop finishes — **whether the last builder Task returned normally OR errored (any error type, including a 529)** — **always** call `get_phase_completion({ featureId, phase: "<phaseName>" })`. This single deterministic call is the source of truth for the branch; never infer "phase done" from the fact that the builder returned. (`null` ⇒ unknown phase name — report phase-not-found and stop, as a `list_phases` miss would.) Then branch on `complete`:
   ...
   - **`complete === false`** (mid-phase stop, or a builder crash with tasks still `todo`/`in_progress`) → **do not** auto-fire the walkthrough, **do not** `clear_active_build()` (the build is still in flight; a re-entered session must resume this phase — preserves the marker's in-flight semantics). Go to step 9 and report the partial state: which tasks landed (`doneCount`/`taskCount`), the failing task id, and the error **verbatim**.
...
9. **Report.**
   ...
   - If step 8 auto-created a walkthrough: its doc id + file path (`walkthroughs/<slug>/phase-<phaseName>.md`), `draft` awaiting review. If one already existed: say so, point at `/specmanager-walkthrough <feature> <phaseName>`. **If `isSinglePhase`**: report this per-phase walkthrough as the **terminal** artifact — approving it ships the feature; never suggest or attempt a `final` roll-up.
   ...
   - **Mid-phase stop** (`get_phase_completion` returned `complete === false`): report the stop only — there was no sync prompt, so say nothing about syncing. Surface the failing task id and the error verbatim. Don't retry. The active-build marker stays set so a re-entered session resumes the phase.
...
- Don't infer "phase done" from the builder returning — **always** call `get_phase_completion` after the loop (return or error) and branch on `complete`. This is the P1 fix: the post-phase pipeline must run off persisted task state, not the builder's exit path, so a 529 after the last task still fires the walkthrough + doc-sync.
- Don't `clear_active_build()` on `complete === false` — the build is in flight; only clear on the terminal `complete === true` (done) or `blocked` paths.
