...
updatedAt: '2026-05-29T10:04:27.441Z'
---
## Problem
...
The hard part is that a partial delivery is often *intentional*. A multi-phase plan may deliberately ship a rough version of a screen now and refine it in a later phase. A naive conformance check would flag those deferred refinements as defects, drowning real gaps in noise. The check must understand the *whole plan* and suppress gaps that a later scheduled phase will close.

## Users

Single user: the local developer driving SpecManager on their own machine. They run `/specmanager-execute <feature> <phase>` to build a phase and own all approvals. There is no team, no remote, no auth (board binds to `127.0.0.1` only).
...
- "Don't nag me about things a later phase is already going to fix."
...
- Be **later-phase-aware**: suppress (but still surface, annotated) gaps that a later phase of the same feature is scheduled to close.
- Present gaps to the user via **AskUserQuestion** and fix only the gaps they confirm.
...
- Be **re-runnable on demand** as a standalone command, independent of a fresh build.
...
- **Not an auto-fixer.** Nothing is changed without explicit user confirmation per gap.
...
- `/specmanager-execute <feature> <phase>` orchestrates the builder via `Task`, after checking the plan is approved. It is the natural place to invoke a post-build check once the builder returns.
...
### FR1 — Dedicated checker subagent
The conformance check runs in a **new dedicated subagent** (e.g. `design-checker`), not bolted onto the builder. The builder remains pure. The checker has the tools it needs (Read/Glob/Grep, the Chrome MCP tools, SpecManager read tools, and whatever it needs to launch/reach the running app), kept as tight as the job allows.

### FR2 — Orchestration and standalone invocation
- `/specmanager-execute` invokes the checker via `Task` **after** the builder returns "phase complete" and after the phase's commits exist.
- The same check is exposed as a **standalone, re-runnable command** so the user can run conformance against any built phase without rebuilding.

### FR3 — Trigger gate (all conditions required; otherwise skip)
The check runs only if **all** hold:
1. An **approved design doc** exists for the feature (`mockups.html` approved).
2. The phase **actually touched UI** (detection mechanism is an open question — see Open questions).
...
- Cross-check observed computed styles against `./docs/DESIGN.md` tokens where feasible.

(Screen→route mapping is an open question — see Open questions.)

### FR6 — Later-phase awareness (load-bearing)
...
Because this is LLM judgment, the checker must **always show its reasoning** so the user can override a suppression. This is precisely why the final step is a confirmation question, not an auto-fix.

### FR7 — Polish checklist output
The check produces a structured checklist. Each item carries:
- `screen`
- `dimension` (layout / spacing / type / color / component / state)
- `expected` vs `observed`
- `severity`
- `deferred?` + which phase covers it (if suppressed)
- `suggestedFix`

### FR8 — Interaction model (AskUserQuestion)
- Use **AskUserQuestion with `multiSelect`**, grouping gaps into a single question ("which of these to fix now?") rather than one question per item (the 4-option cap makes per-item prompting noisy).
- Optionally a **second** `multiSelect` for deferred-but-overridable items, so the user can promote a deferred gap to "fix now."
- Confirmed gaps get fixed; declined/deferred gaps do not.

### FR9 — Fix application and bookkeeping
- Only **user-confirmed** gaps are fixed.
- Fixes must be **board-honest** and preserve SpecManager's artifact/commit invariant (the exact bookkeeping shape — new tasks vs amendment commits — is an open question; see Open questions).

## User flows

### Flow A — Automatic, after building a UI phase
1. User runs `/specmanager-execute <feature> <phase>`.
2. Builder executes the phase, commits, records artifacts, reports "phase complete."
3. Orchestrator invokes the `design-checker` subagent.
...
6. Checker loads the full plan, classifies each gap as flag-now or deferred (with reasoning).
7. Checker presents an AskUserQuestion `multiSelect` of flag-now gaps (plus optional deferred-override question).
8. User selects which to fix; checker fixes only those and records them board-honestly.
9. Orchestrator's normal "phase done" report proceeds, unaffected.

### Flow B — Standalone re-run
1. User runs the standalone conformance command against an already-built phase.
2. Same as Flow A from step 4 onward. No rebuild occurs.
...
- When run on a UI phase with an approved design, the checker produces a screen-by-screen polish checklist with concrete expected-vs-observed gaps on the named dimensions.
- Gaps that a later phase is scheduled to close are **not** presented as fix-now items by default — they appear as deferred with the covering phase named and reasoning shown.
- The user is asked **once** (grouped `multiSelect`), not per-gap, and only confirmed gaps are changed.
...
- The builder remains unchanged (still no Chrome tools, still stops at the phase boundary).
...
- (Assumption) The board server is already running or launchable to serve `mockups.html` for the "expected" screenshots; if not, that is a skip condition.
- Severity scale and the exact dimension rubric are left to architecture/plan to formalize.

## Open questions

These were debated in brainstorm and are deliberately left for architecture/plan to resolve:

1. **Detecting "the phase touched UI" (FR3.2).** Two options: (a) an explicit planner-stamped `touchesUi` flag on tasks — clean but a `tasks.json` schema change; (b) a file-glob heuristic over the phase's completed-task artifact `files` — no schema change but fuzzier. **Recommendation: heuristic for the MVP.**
2. **Screen→route mapping (FR5).** How does the checker know which app route corresponds to which `sm-screen` mockup? **Recommendation: add an optional `Route:` line to the designer's per-screen `sm-note` convention.**
3. **Fix bookkeeping (FR9).** Confirmed polish items become either (a) new small tasks in the current phase ("Polish: <gap>") — board-honest, preserves the artifact/commit invariant, visible on the board; or (b) amendment commits with no task record — lighter, invisible on the board. **Recommendation: new tasks.**
4. **MVP vs full scope.** (a) Dead-simple MVP: screenshot delivery + mockup, model eyeballs a textual diff, one `multiSelect` question, no token-level checks, no route convention; vs (b) the token-aware + later-phase-aware version up front. **Recommendation: phased — MVP first, with later-phase-awareness (FR6) as the first enhancement.**
